...
import { MintWithAuthResponse } from '@lit-protocol/types';
import { ethers } from 'ethers';
import { LIT_NETWORKS_KEYS } from '@lit-protocol/types';

export const readPkpFromStorage = () => {
...
export const readCapacityTokenIdFromStorage = () => {
  return localStorage.getItem('capacityCreditId');
};

export const readNetworkFromStorage = () => {
  const network = localStorage.getItem('network');
  return network as LIT_NETWORKS_KEYS | null;
};
...
export async function getSessionSigs(
  ...
  if (
    litClient.litContracts.network === LIT_NETWORK.DatilTest ||
    litClient.litContracts.network === LIT_NETWORK.Datil
  ) {
    const capacityCreditId = litClient.getCapacityCreditId();
    if (capacityCreditId) {
      capacityDelegationAuthSig =
        await litClient.litNodeClient.createCapacityDelegationAuthSig({
