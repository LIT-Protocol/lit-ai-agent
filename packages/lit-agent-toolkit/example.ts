...
export const runSubAgent = async (): Promise<string> => {
  ...
  try {
    ...
    const firstOwnerWallet = new ethers.Wallet(
      ETHEREUM_PRIVATE_KEY_1,
      provider
    );
    ...
    if (existingConfig && (await configManager.verifyDeployments(provider))) {
      ...
      console.log(
        "ℹ️ No existing configuration found or verification failed. Setting up new Safe and PKP..."
      );
      ...
      const pkpToolsContract = await deployToolsContract(
        firstOwnerWallet,
        safeAddress
      );
    ...
    if (pkpBalance.lt(minimumPkpBalance)) {
      console.warn(`⚠️  Warning: PKP balance is low. Any action is likely to fail due to insufficient funds, especially for gas.
        Has ${ethers.utils.formatEther(
          pkpBalance
        )} ETH, recommended at least 0.00001 ETH`);
    }
  ...
  try {
    const requiredActions = [
      ...
      },
      {
        ...
        description:
          "A Lit Action that sends ETH to a trusted wallet. The AI must provide: amountIn, recipientAddress",
      },
      {
        ipfsCid: "QmSNtQAbYLc5pDrGEjAeUqaih1657V7ht4RJe2mwKBrmWQ",
        description:
          "A Lit Action that unwraps WETH to ETH. The AI must provide: amountIn",
      },
    ];

    for (const requiredAction of requiredActions) {
      const permittedActions = await getPermittedTools(
        pkpToolsContract,
        provider
      );
      const actionExists = permittedActions.some(
        (action: any) =>
          action.ipfsCid === requiredAction.ipfsCid && action.permitted
      );
      ...
      console.log(
        `Permitted action ${requiredAction.ipfsCid} added successfully`
      );
    }
  } catch (error) {
